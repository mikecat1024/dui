...
describe("TextField VRT (outlined)", () => {
                ...
                for (const supportingTextOption of supportingTextOptions) {
                  test(`${multilineOption} ${state} ${validity} ${leadingIconOption} ${trailingIconOption} ${prefixTextOption} ${suffixTextOption} ${supportingTextOption}`, async () => {
                    ...
                    const textFieldProps = {
                      ...
                      ...(hasLeadingIcon
                        ? { leadingIcon: <SearchIcon size={outlinedTextField.leadingIcon.size} /> }
                        : {}),
                      ...(hasTrailingIcon
                        ? {
                            trailingIcon: <CancelIcon size={outlinedTextField.trailingIcon.size} />,
                          }
                        : {}),
                    ...
                    try {
                      ...
                      await expect
                        .element(fieldRoot)
                        .toMatchScreenshot(
                          `text-field-outlined-${multilineOption}-${state}-${validity}-${leadingIconOption}-${trailingIconOption}-${prefixTextOption}-${suffixTextOption}-${supportingTextOption}`,
                        );
                    } finally {
                      await user.cleanup();
                      root.unmount();
